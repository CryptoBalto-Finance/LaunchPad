...
// ALL RIGHTS RESERVED
// Baltosafe reserves all rights on this code. You may NOT copy these contracts.
...
/**
  Allows a decentralised presale to take place, and on success creates a baltoswap pair and locks liquidity on Baltosafe.
...
interface IPresaleLockForwarder {
    ...
    function baltoswapPairIsInitialised (address _token0, address _token1) external view returns (bool);
}
...
contract Presale01 is ReentrancyGuard {
  ...
  struct PresaleInfo {
    ...
    uint256 LISTING_RATE; // fixed rate at which the token will list on baltoswap
    uint256 START_BLOCK;
    uint256 END_BLOCK;
  ...
  struct PresaleFeeInfo {
    uint256 BALTOSAFE_BASE_FEE; // divided by 1000
    uint256 BALTOSAFE_TOKEN_FEE; // divided by 1000
  ...
  address BALTOSAFE_DEV_ADDRESS;
  IBaltoswapFactory public BALTO_FACTORY;
  IWETH public WETH;
  ...
  constructor(address _presaleGenerator) public {
    ...
    BALTO_FACTORY = IBaltoswapFactory();
  ...
  function init2 (
    ...
    uint256 _baltosafeBaseFee,
    uint256 _baltosafeTokenFee,
    ...
    address payable _referralAddress
      ...
      require(msg.sender == PRESALE_GENERATOR, 'FORBIDDEN');
      // require(!PRESALE_LOCK_FORWARDER.baltoswapPairIsInitialised(address(_presaleToken), address(_baseToken)), 'PAIR INITIALISED');
      ...
      PRESALE_FEE_INFO.BALTOSAFE_BASE_FEE = _baltosafeBaseFee;
      PRESALE_FEE_INFO.BALTOSAFE_TOKEN_FEE = _baltosafeTokenFee;
  ...
  function userDeposit (uint256 _amount) external payable nonReentrant {
    ...
    bool userHoldsBaltosafeTokens = PRESALE_SETTINGS.userHoldsSufficientRound1Token(msg.sender);
    if (block.number < PRESALE_INFO.START_BLOCK + STATUS.ROUND1_LENGTH) { // 276 blocks = 1 hour
        require(userHoldsBaltosafeTokens, 'INSUFFICENT ROUND 1 TOKEN BALANCE');
    } else if (STATUS.WHITELIST_ONLY && !userHoldsBaltosafeTokens) {
      // Round 2
  ...
  // If the pair already exists on baltoswap and it contains the presale token as liquidity 
  ...
  function forceFailIfPairExists () external {
    ...
    if (PRESALE_LOCK_FORWARDER.baltoswapPairIsInitialised(address(PRESALE_INFO.S_TOKEN), address(PRESALE_INFO.B_TOKEN))) {
        STATUS.FORCE_FAILED = true;
    }
  }
  
  // if something goes wrong in LP generation
  function forceFailByBaltosafe () external {
      require(msg.sender == BALTOSAFE_DEV_ADDRESS);
      STATUS.FORCE_FAILED = true;
  }
  ...
  function addLiquidity() external nonReentrant {
    ...
    uint256 baltosafeBaseFee = STATUS.TOTAL_BASE_COLLECTED.mul(PRESALE_FEE_INFO.BALTOSAFE_BASE_FEE).div(1000);
    
    // base token liquidity
    uint256 baseLiquidity = STATUS.TOTAL_BASE_COLLECTED.sub(baltosafeBaseFee).mul(PRESALE_INFO.LIQUIDITY_PERCENT).div(1000);
    if (PRESALE_INFO.PRESALE_IN_ETH) {
    ...
    // transfer fees
    uint256 baltosafeTokenFee = STATUS.TOTAL_TOKENS_SOLD.mul(PRESALE_FEE_INFO.BALTOSAFE_TOKEN_FEE).div(1000);
    // referrals are checked for validity in the presale generator
    if (PRESALE_FEE_INFO.REFERRAL_FEE_ADDRESS != address(0)) {
        // Base token fee
        uint256 referralBaseFee = baltosafeBaseFee.mul(PRESALE_FEE_INFO.REFERRAL_FEE).div(1000);
        ...
        baltosafeBaseFee = baltosafeBaseFee.sub(referralBaseFee);
        // Token fee
        uint256 referralTokenFee = baltosafeTokenFee.mul(PRESALE_FEE_INFO.REFERRAL_FEE).div(1000);
        TransferHelper.safeTransfer(address(PRESALE_INFO.S_TOKEN), PRESALE_FEE_INFO.REFERRAL_FEE_ADDRESS, referralTokenFee);
        baltosafeTokenFee = baltosafeTokenFee.sub(referralTokenFee);
    }
    TransferHelper.safeTransferBaseToken(address(PRESALE_INFO.B_TOKEN), PRESALE_FEE_INFO.BASE_FEE_ADDRESS, baltosafeBaseFee, !PRESALE_INFO.PRESALE_IN_ETH);
    TransferHelper.safeTransfer(address(PRESALE_INFO.S_TOKEN), PRESALE_FEE_INFO.TOKEN_FEE_ADDRESS, baltosafeTokenFee);
