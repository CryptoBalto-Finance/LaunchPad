...
contract PresaleLockForwarder is Ownable {
    ...
    IBaltoswapLocker public BALTOSWAP_LOCKER;
    IBaltoswapFactory public BALTOSWAP_FACTORY;
    
    constructor() public {
        PRESALE_FACTORY = IPresaleFactory();
        BALTOSWAP_LOCKER = IBaltoswapLocker();
        BALTOSWAP_FACTORY = IBaltoswapFactory();
    }
    ...
    */
    function baltoswapPairIsInitialised (address _token0, address _token1) public view returns (bool) {
        address pairAddress = BALTOSWAP_FACTORY.getPair(_token0, _token1);
    ...
    function lockLiquidity (IERC20 _baseToken, IERC20 _saleToken, uint256 _baseAmount, uint256 _saleAmount, uint256 _unlock_date, address payable _withdrawer) external {
        ...
        address pair = BALTOSWAP_FACTORY.getPair(address(_baseToken), address(_saleToken));
        if (pair == address(0)) {
            BALTOSWAP_FACTORY.createPair(address(_baseToken), address(_saleToken));
            pair = BALTOSWAP_FACTORY.getPair(address(_baseToken), address(_saleToken));
        }
        ...
        IBaltoswapPair(pair).mint(address(this));
        uint256 totalLPTokensMinted = IBaltoswapPair(pair).balanceOf(address(this));
        require(totalLPTokensMinted != 0 , "LP creation failed");
    
        TransferHelper.safeApprove(pair, address(BALTOSWAP_LOCKER), totalLPTokensMinted);
        uint256 unlock_date = _unlock_date > 9999999999 ? 9999999999 : _unlock_date;
        BALTOSWAP_LOCKER.lockLPToken(pair, totalLPTokensMinted, unlock_date, address(0), true, _withdrawer);
    }
